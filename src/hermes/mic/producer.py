...
from widgets import LinePlotVisualizer
...
class MicrophoneStreamer(Producer):
    ...
    def __init__(
        ...
        self._chunk_size = (
            2048  # number of samples to acquire before running the callback function
        )
        array_decoding_codes = (
            {  # based on https://docs.python.org/3/library/array.html
                1: "B",
                2: "h",
                4: "l",
                8: "q",
    ...
    def _get_available_audioDevices_list_str(self):
        ...
        for (
            audioDevice_index,
            audioDevice_info,
        ) in self._input_audioDevices_info.items():
            msg += "\n ID %2d: %s" % (audioDevice_index, audioDevice_info.get("name"))
        msg += "\nOutput audio devices:"
        for (
            audioDevice_index,
            audioDevice_info,
        ) in self._output_audioDevices_info.items():
            msg += "\n ID %2d: %s" % (audioDevice_index, audioDevice_info.get("name"))
        return msg
    ...
    def _connect(self, timeout_s=10):
        ...
        if self._device_names_withAudioKeywords is not None:
            for (
                ...
                for (
                    device_name,
                    audioKeyword,
                ) in self._device_names_withAudioKeywords.items():
                    if audioKeyword in audioDevice_name:
                        self._device_names_withAudioIndexes[device_name] = (
                            audioDevice_index
                        )
            # Check that all desired devices were found
            if len(self._device_names_withAudioKeywords) != len(
                self._device_names_withAudioIndexes
            ):
                raise AssertionError(
                    "Found %d/%d specified microphones.  The following devices are available:\n%s\n\n"
                    % (
        ...
        if len(self._device_names_withAudioIndexes) == 0:
            ...
            if len(self._device_names_withAudioIndexes) == 0:
                (audioDevice_index, audioDevice_info) = list(
                    self._input_audioDevices_info.items()
                )[0]
                audioDevice_name = audioDevice_info.get("name")
                device_name = self._device_name_from_audioDevice_name(audioDevice_name)
                self._device_names_withAudioIndexes[device_name] = audioDevice_index
        ...
        msg = "Recording from the following %d audio devices" % len(
            self._device_names_withAudioIndexes
        )
        for (
            device_name,
            audioDevice_index,
        ) in self._device_names_withAudioIndexes.items():
            msg += "\n  Audio device ID %2d (%s) as stream %s" % (
                audioDevice_index,
                self._input_audioDevices_info[audioDevice_index].get("name"),
                device_name,
            )
        ...
        for (
            ...
            self._audio_streams.append(
                self._pyaudio.open(
                    ...
                    stream_callback=lambda *args, device_name=device_name, audioDevice_index=audioDevice_index: self._mic_data_callback(  # bind it to the current value  # bind it to the current value
                        device_name, audioDevice_index, *args
                    ),
                )
            )
            ...
            self.add_stream(
                ...
                data_notes=OrderedDict(
                    [
                        (
                            "Description",
                            "Raw audio data recorded from the microhpone."
    ...
    def _mic_data_callback(
        self, device_name, audioDevice_index, data, frame_count, time_info, status
    ):
        # Timestamp the data and advance the chunk counter.
        time_s = time.time()
        self._num_chunks_received[audioDevice_index] = (
            1 + self._num_chunks_received[audioDevice_index]
        )
...
if __name__ == "__main__":
    ...
    microphone_streamer = MicrophoneStreamer(
        print_status=print_status,
        print_debug=print_debug,
    ...
    for device_name in microphone_streamer.get_device_names():
        num_timesteps = microphone_streamer.get_num_timesteps(
            device_name, "chunked_data"
        )
        num_frames = num_timesteps * microphone_streamer._chunk_size
        print(
            "  %s: Nchunks=%d, chunkRate=%0.2f | Nframes=%d, frameRate=%0.2f"
            % (
                device_name,
                num_timesteps,
                (num_timesteps / duration_s),
                num_frames,
                (num_frames / duration_s),
            )
        )
    ...
    for device_name in microphone_streamer.get_device_names():
        ...
        wf.setsampwidth(
            microphone_streamer._pyaudio.get_sample_size(microphone_streamer._format)
        )
        ...
        print(
            "from written WAV file: %d frames, %0.2fHz, duration %gs"
